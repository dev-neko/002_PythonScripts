...
try:
	...
	try:
		...
		logger.warning('予約指定が無いため、手続きを終了します。')
		sys.exit()

	##########################
	# 確認完了したら削除する #
	##########################
	# res_con=['上井草スポーツセンター','人工芝庭球場Ａ面','6月8日(火)','6時00分～8時00分','4月22日(木)']
	...
	logger.info('Herokuサイトからの情報取得が完了したため、8時29分30秒まで待機します。')
	...
	def before_reserve():
		...
		logger.info('8時29分30秒になったため、事前手続きを開始します。')
		...
		while True:
			if res_day in BeautifulSoup(driver.page_source,'html.parser').text:
				logger.info('予約希望日が見つかりました。')
				break
			else:
				try:
					driver.find_element_by_xpath("//*[@value='  次の週  ']").click()
				except selenium.common.exceptions.NoSuchElementException:
					logger.warning("予約希望日が見つからないため、手続きを終了します。")
					sys.exit()

		# 事前手続きの時点で予約希望日の空き枠があるかを確認
		bs4obj=BeautifulSoup(driver.page_source,'html.parser')
		hope_day_time_list=[]
		for item in bs4obj.form(text=True):
			tmp_item=re.sub(r'\(.*?\)','',item.replace("\t","").replace("\n",""))
			if tmp_item!='' and tmp_item!='ご希望日を選択してください':
				hope_day_time_list.append(tmp_item)
		logger.debug(f'ページ内の空き枠情報\n{hope_day_time_list}')
		free_border=hope_day_time_list[hope_day_time_list.index(res_day)+1]
		logger.info(f'予約希望日の空き枠情報：{res_day}{free_border}')
		if bool(re.search(r'\d',free_border)):
			logger.info('予約希望日に空き枠を確認出来たため、8時30分まで待機します。')
		else:
			logger.warning('事前手続き時点で既に枠が全て埋まっているため、手続きを終了します。')
			sys.exit()
	...
	schedule.every().day.at("08:29:30").do(before_reserve)
