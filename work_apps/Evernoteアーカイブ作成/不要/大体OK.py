...
# note_title_dict={"【済】特別儀式／鑑定":['おおお','えええ']}
...
for notebook in notebook_list:
	...
	if notebook.name in title_in or notebook.name in title_not_in:
		...
		for hope_note_title in note_title_dict[notebook.name]:
			...
			if note_count>1:
				print("------------------------------")
				print(f'検索対象のノートのタイトル：{hope_note_title} が {note_count} 個あるためアーカイブを更新せずに終了します。')
				sys.exit()
			elif note_count==0:
				print("------------------------------")
				print(f'検索対象のノートのタイトル：{hope_note_title} が見つからないためアーカイブを更新せずに終了します。')
				sys.exit()
			for notes_metadata in notes_metadata_list:
		...
		print("------------------------------")
		print(f'更新したアーカイブの内容：{add_content}')