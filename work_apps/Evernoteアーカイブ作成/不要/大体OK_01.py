...
# note_title_dict={"【済】Coaching Free Salon(CFS)":['幸せとは選択することから']}
		...
		for hope_note_title in note_title_dict[notebook.name]:
			...
			if note_count>1:
				...
				if hope_note_title==note_other.title:
					...
					other_content=note_other.content.replace('<?xml version="1.0" encoding="UTF-8"?>','').replace('</en-note>','').replace('<en-note>','').replace('<!DOCTYPE en-note SYSTEM "http://xml.evernote.com/pub/enml2.dtd">','')
